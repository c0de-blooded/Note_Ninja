...
def topics_excavator(text):
    GOOGLE_API_KEY = 'AIzaSyA85gibyQMuDNkJUi4bNTCRC378XpROgUQ'
    genai.configure(api_key=GOOGLE_API_KEY)

    model = genai.GenerativeModel('gemini-pro')
    response = model.generate_content("Extract the general topics from this text. Make sure it's only the global topics, not anything specific:: " + text)
